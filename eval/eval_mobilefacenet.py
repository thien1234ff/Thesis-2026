# ============================================================
# FULL EVAL SCRIPT: FP32 vs Q6 (MobileFaceNet)
# ============================================================
...
@torch.no_grad()
def extract_embeddings(model, images, batch_size=128, use_flip=True):  # MobileFaceNet nhẹ
    model.eval()
    embeddings = []
...
def load_backbone(weight_path, bit="fp32"):
    ...
    if bit == "fp32":
        ...
        model = quantize_model(
            MobileFaceNet(embedding_size=512),
            weight_bit=bit,
            act_bit=bit
        )
...
Q6_PATH = "/kaggle/input/datasets/hhongeeee/beta300/mobilefacenet_q6_epoch29.pth"

eval_model(FP32_PATH, "FP32", bit="fp32")
eval_model(Q6_PATH, "Q6", bit=6)